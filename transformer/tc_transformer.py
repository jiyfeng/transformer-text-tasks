...
class TCTransformer(nn.Module):
    def __init__(self, emb, heads, depth, seq_length, num_tokens, num_classes, dropout=0.0):
        super().__init__()

        self.num_tokens = num_tokens
        self.token_emb = nn.Embedding(num_embeddings=num_tokens, embedding_dim=emb)
        self.pos_emb = nn.Embedding(num_embeddings=seq_length, embedding_dim=emb)  # position_encoding can be used instead of embedding
        self.dropout = nn.Dropout(dropout)

        # Sequence of transformer blocks that does the heavy lifting
        trans_blocks = []
        for i in range(depth):
            trans_blocks.append(TransformerBlock(emb=emb, heads=heads))
